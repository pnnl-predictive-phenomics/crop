...
def s_table():
    ...
    return pd.DataFrame(S_dict, index=S_index)


##### Tests #####
### COBRA model tests
def test_load_cobra_model_from_stoichiometry_table(s_table: pd.DataFrame):
    # load reactions, metabolites, stoichiometry from S_table dataframe
    cobra_model = model_from_stoich_matrix(s_table)
    test_cobra_model_metabolites(cobra_model)
    test_cobra_model_reactions(cobra_model)
    test_cobra_model_stoichiometric_matrix(cobra_model)
...
# test growth objective set correctly (e.g. C_int->C_SNK)
def test_growth_objective():
    actual = ...  # get growth objective for ABC toy model 3
    expected = ...  # growth objective for ABC toy model 3
    raise NotImplementedError
...
def test_get_steady_state_dual_constraint(model, phenotype_observations, growth_objective):
    actual_constraint = get_steady_state_dual_constraint(
        model, phenotype_observations, growth_objective
    )
